...
VS Code with Claude Code, so no session starts from zero.
...
opponent later); score to 5; no continuous ball acceleration (one step up
on the first paddle hit is the deliberate exception). Nothing open right
now.
